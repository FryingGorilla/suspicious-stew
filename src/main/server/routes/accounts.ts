...
router.post('/add', async (req, res) => {
	const {email, configPath} = req.body;
	if (!email) return res.status(400).json({error: 'No email provided'});

	const uuid = await Account.createAccount(email, configPath, (codeRes) => {
		...
		if (!res.headersSent) res.status(202).json({message: 'OK', response: codeRes});
	});
	if (uuid != null) await Server.get().addAccount(uuid);
	if (!res.headersSent) res.status(uuid != null ? 200 : 500).json({message: 'OK', accounts: mapAccounts()});
});
