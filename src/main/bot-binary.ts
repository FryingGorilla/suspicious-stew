...
export async function downloadBotBinary(forceDownload?: boolean) {
  ...
  botBinaryPath = await downloadLatest(
...
const downloadLatest = async (nameRegex: RegExp, forceDownload?: boolean) => {
  ...
  if (!forceDownload) {
    ...
    if (!current && !asset)
      throw new Error(`No matching asset found for ${nameRegex}`);
    if (current?.name) {
      try {
        const idPath = path.join(BIN_DIR, current.name + ".id");
        const currentId = await fs.promises.readFile(idPath, "utf8");
        if (Number(currentId) === asset.id)
          return path.join(BIN_DIR, current.name);
      } catch (err) {
  ...
  try {
    await fs.promises.writeFile(
      path.join(BIN_DIR, asset.name + ".id"),
      asset.id.toString()
    );
  } catch (err) {
