	...
	const fetchProducts = async (): Promise<HypixelProducts> => {
		...
		if (Date.now() - cache.products.time > cache.products.maxAge) {
			try {
				const {
					data: { products },
				} = await axios<HypixelAPIResponse>(
		...
		if (Date.now() - cache.knownItems.time > cache.knownItems.maxAge) {
			try {
				const res = await axios(
					new URL("/api/skyblock/items", globals.API_URL).toString(),
					{
	...
	return {
		solve: async ({
			...
			const getItems = async () => {
				...
				return knownItems
					?.filter((item) => {
						...
						if (
							orders.some(
								({ productId, type }) => productId === item.id && type === "buy"
							)
						) {
							return false;
						}
						if (
							filter.blacklist.some((pattern) =>
								new RegExp(pattern).test(item.id)
							)
						) {
							return false;
						}
						if (
							filter.whitelist.length &&
							!filter.whitelist.some((pattern) =>
								new RegExp(pattern).test(item.id)
							)
						) {
							return false;
					...
					?.map(({ id, name, maxStack }) => {
						...
						const hours =
							((hour[id]?.at(-1)?.time ?? 0) - (hour[id]?.at(0)?.time ?? 0)) /
							1000 /
							60 /
							60;

						const buyPrice = product?.sell_summary[0]?.pricePerUnit
							? product?.sell_summary[0]?.pricePerUnit + 0.1
							: 0;
						const sellPrice = product?.buy_summary[0]?.pricePerUnit
							? product?.buy_summary[0]?.pricePerUnit - 0.1
							: 0;
						const hourlyBuyMovement =
							(product?.quick_status?.buyMovingWeek ?? 0) / 7 / 24;
						const hourlySellMovement =
							(product?.quick_status?.sellMovingWeek ?? 0) / 7 / 24;
						...
						const usage = orders.reduce(
							(prev, cur) =>
								prev +
								(cur.productId === id
									? (cur.amount ?? 0) * (cur.price ?? 0)
									: 0),
							0
						);
						const usageLeft = Math.min(
							filter?.maxUsageProduct
								? filter.maxUsageProduct - usage
								: Infinity,
							budget / orderCount
						);
					...
					.filter((item) => {
						if (item.profitability == null || item.profitability < 0)
							return false;
						if (item.amount < 1) return false;

						if (
							filter.maxHourlyUndercuts <
							Math.max(item.hourlyBuyUndercuts, item.hourlySellUndercuts)
						)
							return false;

						if (
							Math.min(item.hourlyBuyMovement, item.hourlySellMovement) <
							filter.minMovement
						)
							return false;
						...
						if (
							Math.abs(item.avgSellPrice - item.sellPrice) >
							item.sellPrice * filter.maxDiffDay
						)
							return false;
						if (
							Math.abs(item.avgBuyPrice - item.buyPrice) >
							item.buyPrice * filter.maxDiffDay
						)
							return false;
			...
			const solve = <T>(
				...
				const dp = new Array(itemCount + 1)
					.fill(null)
						...
						for (let j = itemCount; j > 0; j--) {
							if (j > 0 && w >= currentWeight) {
								...
								if (option2 >= option1) {
									dp[j][w] = option2;
									selectedItems[j][w] = [
										...selectedItems[j - 1][w - currentWeight],
										i,
									];
								}
							}
				...
				return {
					...
					selectedIndices: selectedItems[itemCount][limit].map(
		...
		getProducts: async (): Promise<Product[]> => {
			const [knownItems, products] = await Promise.all([
				fetchKnownItems(),
				fetchProducts(),
			]);
			...
			return knownItems
				...
				.map(({ id, maxStack, name }) => {
					...
					const profitability =
						margin * Math.min(hourlyBuyMovement, hourlySellMovement);
