...
export default class Config {
    ...
    private constructor() {
        const options = {
            ...
            botPath: getArgValue(["bot", "b", "botPath"]),
        };
    ...
    public async prompt() {
        const dontQuery = getArgValue(
            ["dontQuery", "q", "noQuery", "nq"],
            "true"
        );
        if (!dontQuery) {
            await inquirer
        ...
        if (!globals.IS_IN_DEV) {
            if (this.options["botPath"] === undefined) {
                ...
                logger.error(
                    `Provided bot binary does not exist at '${this.options.botPath}'`
                );
                isValid = false;
            }
        }
        ...
        if (!validHosts.includes(this.options.host)) {
            logger.error(
                `'host' must be one of ${validHosts
                    .map((s) => `'${s}'`)
                    .join(", ")}`
            );
            isValid = false;
