...
export default class BazaarFlipper {
				...
				if (this.isScheduled()) {
					if (this.isInTimeout) {
						...
						this.manager.postNotification(
							"Timeout ended",
							"Scheduled timeout has ended, logging back in",
							2
						);
					...
					this.manager.postNotification(
						"Timeout",
						"Scheduled timeout has started, disconnecting",
						2
					);
					this.isInTimeout = true;
					if (this.manager.onlineStatus === "connecting")
						await this.manager.waitForBotEvent("spawn");
	...
	stop(): void {
		...
		this.startingUsedDailyLimit = undefined;
		this.cycles = 0;
	...
	constructor(public manager: BotManager) {
		...
		manager.bot.on(
			...
			async (chatMessage: ChatMessage, position: string) => {
				...
				if (
					message.startsWith(
						"This server is too laggy to use the Bazaar, sorry!"
					)
				) {
				...
				const joinedName = /^([a-zA-Z0-9_]+) joined SkyBlock./
					.exec(message.toString())
					?.at(1);
				const leftName = /^([a-zA-Z0-9_]+) left SkyBlock./
					.exec(message.toString())
					?.at(1);
	...
	async main() {
		...
		if (this.manager.location !== "island") {
			return await this.changeActivity("failsafe", async () => {
				this.manager.postNotification(
					`Failsafe activated`,
					`Current location: ${this.manager.location}`,
					2
				);
				logger.debug(
					`Failsafe activated: Current location: ${this.manager.location}`
				);

				const message =
					this.manager.location === "limbo"
						? "/l"
		...
		if (!manager.hasCookie) {
			...
			(await this.getCookieBuffTime()) < 24 * 60 * 60 * 1000 &&
			manager.config.options.general.autoCookie
		) {
			logger.debug("Less than 2 days left of cookie buff");
			const cookie = this.manager.bazaar.getProduct("BOOSTER_COOKIE");
			if (!cookie)
				return logger.debug("Failed to get BOOSTER_COOKIE bazaar products");
			if (cookie.instantBuyPrice * 2 > this.manager.getPurse())
				logger.debug("Not buying a cookie: not enough money");
			else return this.buyCookie();
		}
		...
		if (this.startingUsedDailyLimit === undefined)
			this.startingUsedDailyLimit = manager.bazaar.usedDailyLimit;
		...
		logger.debug([
			...
			`Inventory (${formatNumber(
				...
				manager.bazaar
					.getBazaarProductsFromInv()
					.map(({ product, amount }) => ({ id: product.id, amount }))
			)}`,
			...
			`Profit / h: ${formatNumber(
				(total - this.startingTotal) /
					(this.timer.getElapsedTime() / 1000 / 60 / 60)
			)}`,

			`Daily limit: ${formatNumber(
				manager.bazaar.usedDailyLimit
			)} / ${formatNumber(manager.bazaar.getTrueLimit())}`,
			`Cycles: ${this.cycles}`,
		]);
		...
		await (async () => {
			if (manager.bazaar.getRemainingOrderSpace("sell") > 0) {
				...
				const order = manager.bazaar.orders.find(
					(e) =>
						!last || e.productId === last.id || (e.filled && e.type === "sell")
				);

				if (order) {
					if (order.undercutAmount === undefined || order.amount === undefined)
						throw Error("Weird order");
					if (order.filled) {
						if (order.type === "sell") return manager.bazaar.claimOrder(order);
						else {
							if (manager.bazaar.fitsInv(order))
								return manager.bazaar.claimOrder(order);
							return manager.bazaar.flipOrder(order);
						}
					}
					if (
						order.undercutAmount / order.amount >
						manager.config.options.orders.relistRatio
					)
				...
				const freeSpace = (order?: Order): number => {
					if (!order) return 0;
					if (!order.filled && !manager.bazaar.fitsInv(order)) return 0;
					if (
						order.filled &&
						order.type === "buy" &&
						!manager.bazaar.fitsInv(order)
					)
						return 0;
					if (!order.amount) return 0;
					const orders = manager.bazaar.orders
						.filter((o) => o.productId === order.productId && o.amount)
					...
					for (const order1 of orders) {
						if (order1.type === "sell" && order1.filled) {
							count++;
							continue;
						}

						sum.amount! += order1.amount!;
						if (manager.bazaar.fitsInv(sum)) count++;
					}
					return count;
				};

				const orders = manager.bazaar.orders.sort((a, b) => {
					if (a.type !== b.type) {
						if (a.type === "sell") return -1;
						return 1;
					}
					return (
						freeSpace(b) - freeSpace(a) ||
						(a.filled !== b.filled ? (a.filled ? -1 : 1) : 0)
					);
				});
				if (freeSpace(orders[0]) === 0) {
					const order = manager.bazaar.orders
						.filter((o) => o.amount && o.price)
						.sort((a, b) => {
							return a.amount! * a.price! - b.amount! * b.price!;
						})[0];
					if (order.filled) await manager.bazaar.claimOrder(order);
					else await manager.bazaar.cancelOrder(order);
					return this.sellInvAndStash(true);
				}
				let lastId: null | string = null;
				for (const order of orders) {
					if (!freeSpace(order)) continue;
					if (lastId && lastId !== order.productId) break;
					lastId = order.productId;
					if (order.filled) await manager.bazaar.claimOrder(order);
					else await manager.bazaar.cancelOrder(order);
				}
			}
			// Create buy orders and wait
			{
				...
				if (manager.bazaar.getRemainingOrderSpace("buy") > 0 && budget > 0) {
					...
					logger.debug([
						...
						`Max usage: ${formatNumber(
							manager.config.options.general.maxUsage
						)}`,
						`Spent: ${formatNumber(manager.bazaar.getSpent())}`,
					...
					for (const order of orders)
						await retry(5, () => manager.bazaar.createOrder(order));
				} else {
					logger.debug([
						...
						`Remaining daily limit: ${formatNumber(
							manager.bazaar.getRemainingLimit()
						)}`,
						`Remaining usage: ${formatNumber(
							manager.config.options.general.maxUsage -
								manager.bazaar.getSpent()
						)}`,
					]);
				}
				...
				const avgUsage =
					(manager.bazaar.usedDailyLimit - (this.startingUsedDailyLimit ?? 0)) /
					this.cycles;
				const avgDuration =
					(Math.min(
						this.timer.getElapsedTime(),
				...
				const delay =
					(remainingTime - remainingCycles * avgDuration) /
					(remainingCycles - 1);
				...
				logger.debug([
					...
					)} (starting: ${formatNumber(this.startingUsedDailyLimit ?? 0)})`,
	...
	async sellInvAndStash(instantSell?: boolean) {
		...
		while (
			...
			try {
				...
				if (message.includes("all") || message.includes("isn't holding any"))
					isEmpty = true;
				...
				if (instantSell) await this.manager.bazaar.instantSell();
					...
					await this.manager.bazaar.createOrder({
				...
				if (
					isEmpty &&
					this.manager.bazaar.getBazaarProductsFromInv().length === 0
				)
					break;
				fails = 0;
			} catch (err) {
	...
	async checkForOnlineMembers(): Promise<boolean> {
		...
		try {
			...
			if (!bot.currentWindow)
				throw new Error("Failed to open profile management");
			const selected = bot.currentWindow
				?.containerItems()
				.findIndex((item) =>
					getCleanLore(item).includes("You are playing on this profile!")
				);
	...
	coopFailsafe() {
		return this.changeActivity("coopFailsafe", async () => {
			...
			this.manager.postNotification(
				"Co-op failsafe",
				`Some Co-op members are online (${this.onlineMembers.join(
					", "
				)}), attempting to instant-sell everything...`,
				3
			);
			await this.manager.sendChat(
				`/cc hellooo ${this.onlineMembers.join(
					", "
				)}! pls don't touch my orders!!!!`
			);
			...
			while (
				...
				this.manager.bazaar.orders.sort(
					(a, b) =>
						(b.amount ?? 0) * (b.price ?? 0) - (a.amount ?? 0) * (a.price ?? 0)
				);
	...
	buyCookie() {
		return this.changeActivity("buyingCookie", async () => {
			...
			try {
				...
				this.manager.postNotification(
					"Auto Cookie",
					"Attempting to buy a new Booster Cookie",
					3
				);

				if (findItemSlot(boosterCookie.name, bot.inventory, true) === -1) {
					if (this.manager.isInventoryFull()) {
						await this.manager.bazaar.instantSell(
							this.manager.bazaar.getBazaarProductsFromInv()[0]?.product
						);
					}
				...
				if (slot !== bot.quickBarSlot)
					await bot.moveSlotItem(
						slot,
						bot.inventory.hotbarStart + bot.quickBarSlot
					);
				await wait(250);
				bot.activateItem();
				bot.deactivateItem();
	...
	async getCookieBuffTime() {
		if (this.cookieBuffTime)
			return this.cookieBuffTime - (Date.now() - this.checkTime);
	...
	serialize(): FlipperUpdateData {
		return {
			...
			startingUsedDailyLimit: this.startingUsedDailyLimit,
			profit: this.startingTotal
				? this.manager.bazaar.lastValidated.total - this.startingTotal
				: undefined,
			cookieBuffTime: this.cookieBuffTime
				? this.cookieBuffTime - (Date.now() - this.checkTime)
				: 0,
