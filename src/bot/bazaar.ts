...
export default class Bazaar {
        ...
        manager.bot.on(
            ...
            (message: ChatMessage) => {
                if (
                    ...
                    message
                        .toString()
                        .startsWith("You consumed a Booster Cookie!")
                ) {
    ...
    async loadLimits() {
        if (!this.manager.account.email) return;
        if (
            !existsSync(globals.ACCOUNT_LIMIT_CACHE(this.manager.account.email))
        )
            return;
        ...
        try {
            ...
            if (order.type === "buy") {
                ...
                await this.manager.clickItem(
                    ...
                    this.manager.writeToSign(
                        String(
                            Math.floor(Math.min(order.amount ?? 1, maxAmount))
                        )
                    )
                );
                await this.manager.waitForBotEvent("windowOpen");
            ...
            if (lore.includes("Placing orders is on cooldown!")) {
                ...
                if (this.orders.length) {
                    this.manager.config.options.orders.maxOrders =
                        this.orders.length;
                ...
                if (isNaN(price)) {
                    price =
                        (order.type === "buy"
                            ...
                            : product.sellPrice) ?? 1;
                    logger.debug(
                        `Failed to find price for ${JSON.stringify(
                            order
                        )} ${lore}`
                    );
                }
                ...
                if (isNaN(amount)) {
                    ...
                    logger.debug(
                        `Failed to find amount for ${JSON.stringify(
                            order
                        )} ${lore}`
                    );
                }
    ...
    async cancelOrder(order: Order) {
        ...
        try {
            ...
            while (
                ...
                if (this.manager.isInventoryFull() && order.type === "buy")
                    return;
    ...
    async claimOrder(order: Order) {
        ...
        try {
            ...
            throw new Error(
                `Failed to claim order ${JSON.stringify(order)}: ${err}`
            );
        }
    }
    ...
    async flipOrder(order: Order) {
        ...
        try {
            ...
            if (isNaN(topPrice)) {
                ...
                logger.debug(
                    `Failed to find top price for ${JSON.stringify(
                        order
                    )} ${getCleanLore(item)}`
                );
            }
            ...
            throw new Error(
                `Failed to flip order ${JSON.stringify(order)}: ${err}`
            );
        }
    }
    ...
    async instantSell(product?: Product) {
        ...
        try {
            if (product) {
                this.usedDailyLimit +=
                    (this.getBazaarProductsFromInv().find(
                        (e) => e.product === product
                ...
                this.usedDailyLimit += this.getBazaarProductsFromInv().reduce(
                    (sum, { product, amount }) =>
                        product.instantSellPrice * amount,
                    0
                );
                await this.openBz();
    ...
    getOrdersWorth() {
        return this.orders.reduce(
            (sum, order) => sum + this.getValue(order),
            0
        );
    }
    ...
    async updateProducts() {
        this.products = await this.manager.waitForReply<
            Product[],
            "get-products"
        >("get-products", null);
    }
    ...
    findOrderSlot(order: Order): number {
        ...
        for (let slot = 0; slot < bot.currentWindow.inventoryStart; ++slot) {
            ...
            logger.debug(
                `Found order ${JSON.stringify(order)} in slot ${slot}`
            );
            return slot;
        }
        logger.debug(`Failed to find order ${JSON.stringify(order)})`);
    ...
    getUndercutAmount(order: Order): number {
        ...
        const competingOrders = (
            ...
            (a) =>
                !sameOrders.find(
                    (b) => a.amount === b.amount && a.price === b.price
                )
        );
    ...
    getBazaarProductsFromInv(): { product: Product; amount: number }[] {
        ...
        bot?.inventory?.items()?.forEach((item) => {
            let id = getNbt(item, "ExtraAttributes", "id") as string;
            if (id == null) return;

            if (id === "ENCHANTED_BOOK") {
                const enchants = getNbt(
                    item,
                    "ExtraAttributes",
                    "enchantments"
                ) as Record<string, { type: "int"; value: number }>;
                const enchant = Object.entries(enchants)[0];
                if (!enchant) return;
                id = `ENCHANTMENT_${enchant[0].toUpperCase()}_${
                    enchant[1].value
                }`;
            }
